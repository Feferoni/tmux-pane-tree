...
class Window:
    ...
    def load_panes(self) -> None:
        ...
        for line in output.split('\n'):
            if line:
                parts = line.split('|')
                pane = Pane(parts[0], self, parts[1], parts[2] == '1', parts[3], parts[4], parts[5], parts[6], parts[7])
        ...
        for session in self.sessions:
            for window in session.windows:
                for pane in window.panes:
                    if pane.active and window.active:
                        return pane
        return None
